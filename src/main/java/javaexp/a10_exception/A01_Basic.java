	...
	public static void main(String[] args) {
		...
		1. 오류의 종류
			1) 에러
				...
				정산 실행상태로 돌아갈 수 없음.
			2) 예외(exception)
				사용자의 잘못된 조작 또는 개발자의 잘못된 코딩으로 인한 오류 예외가 발생되면 프로그램 종료
...
//		System.out.println(1/0); // 에러가 발생하면 더 이상 프로세스가 진해아지 않음.
...
//		ex) String name=null; System.out.println(name.toString()); 
		// 위 코드를 try{}catch구문 없이 처리하고, 다시 try{}catch 구문으로 처리하여 예외 프로세스가 어떻게 흘러가는지를 확인하세요.
		...
		2. 실행 후, console창에 발생한 예외를 복사를 한다.
		...
		System.out.println(arr[3]);
			...
			System.out.println("예외 내용 상관 없이 수행");
		}
		System.out.println("프로그래밍 계속 진행");
		// ex) Sysstem.out.println(Integer.parseInt("이십오"));
