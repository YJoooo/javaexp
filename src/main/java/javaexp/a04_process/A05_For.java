...
public class A05_For {
	...
	public static void main(String[] args) {
		...
		for(int cnt=1;cnt<=9;cnt++) {
			System.out.println("5*"+cnt+"="+5*cnt);
		...
		for(int date=1;date<=10;date++) {
			...
			System.out.println(date+"째날 판매 금액: "+kimbap*dateTot);
			totSell += dateTot;
		}
		...
		// ex2) 2 + 4 + 6 .. 10 + 의 합을 출력(출력형식도 처리)하세요.
		int sum3=0;
		...
		int jjajang = 5000;
		for(int cnt=1;cnt<=10;cnt++) {
			System.out.println(cnt+"그릇 "+(cnt*jjajang)+"원");
		}
