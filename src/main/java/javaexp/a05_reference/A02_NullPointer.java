...
public class A02_NullPointer {
	...
	public static void main(String[] args) {
		...
		/*
		# null (stack영역과 heap 영역이 어떻게 사용되는지 알면 됨)
		...
		heap영역에는 할당하지 않고, stack영역에서 선언만 한 상태에서 객체를 초기화시키는 것을 말한다.
		Person p =null;
		...
		System.out.println(f); // 객체는 선언이 되어있지만 heap영역에는 할당되지 않은 상황이라고 null을 표시한다.
		// System.out.println(f.name); NullPointerExceptoion
		// NullPointerException : Pointer(주소값)이 NUll(없는) Exception(에러/예외) 발생했습니다.
		//	- heap영역에 객체가 생성되지 않는 상황에서 객체가 가지고 있는 구성요소를 호출할 때, 발생하는 예외..
